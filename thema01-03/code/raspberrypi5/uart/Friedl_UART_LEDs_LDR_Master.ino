...
#define WRITE_LDR_DELAY   3000    // delay between 2 write LDR commands
...
void loop(void) {
  ...
  if(now - WRITE_LDR_DELAY > lastWrite){
    ...
    writeSerialLDR(WRITE_LDR, sensValue);
    lastWrite = now;
 }
}
...
void writeSerialLDR(byte command, int value) {
    Serial.println(command);
    Serial.println(value);
    softSerial.write(command); 
    softSerial.write((value >> 8) & 0xFF); // send the high byte of the LDR value to slave first
    softSerial.write(value & 0xFF);        // send low byte of LDR value to slave second (MSB first!)
}
